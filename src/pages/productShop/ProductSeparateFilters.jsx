...
const ProductShop = () => {
  ...
  const [filteredByPrice, setFilteredByPrice] = useState(products);
  ...
  const filterByCategory = () => {
    if (categoryName) {
      setFilteredByCategory(products.filter(product => product.category === categoryName));
    } else {
  ...
  const filterByPrice = () => {
    setFilteredByPrice(products.filter(product => product.price >= minPrice && product.price <= maxPrice));
  };
    ...
    <div className="customer-layout">
      <main className="main">
        ...
        <div className="grid grid-cols-4 gap-5">
          ...
          {filteredByCategory
            .filter(product => filteredByPrice.includes(product)) 
            .map((product) => (
              <div key={product.id}>
